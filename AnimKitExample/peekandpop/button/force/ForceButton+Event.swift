...
extension ForceButton {
   ...
   override func pressureChange(with event: NSEvent) {
      ...
      if event.pressureBehavior == NSEvent.PressureBehavior.primaryDeepClick, prevStage != curStage {
         switch (curStage, prevStage) {
         case (0, 1):
            stageChange(Stage.clickUp) // super.onEvent(ForceTouchEvent(ForceTouchEvent.clickUp,prevStage,self,event))
         case (1, 0):
            stageChange(Stage.clickDown) // super.onEvent(ForceTouchEvent(ForceTouchEvent.clickDown,prevStage,self,event))
         case (1, 2):
